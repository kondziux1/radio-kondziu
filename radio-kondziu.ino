...
uint8_t cur_station  = 1;   //current station(nr), will be set later
uint8_t cur_volume   = 21;   //will be set from stored preferences
bool volch = false;
String cheese = "";
...
int radioput(String url){
  for(int x = 0; x < stations.lenght(); x++){
    if(stations[x] == ""){
      stations[x] = url;
      return 0;
    }
